...
void* k_calloc(size_t size){ 
    void* p = KERNEL_SPACE(malloc(size));
    memset(p, 0, size);
    return p;
}

void* k_malloc(size_t size){ 
    return KERNEL_SPACE(malloc(size));
}
...
uint32_t alloc_page(uint32_t req_num_page){
    ...
    remove_page(free_list[alloc_page_order], available_page); // del first node
