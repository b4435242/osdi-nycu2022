...
cpio_ext* cpio_search(char *name){
    cpio_newc_header* header = (cpio_newc_header*)CPIO_BASE;
    cpio_ext *info = k_malloc(sizeof(cpio_ext));
    while(1) {
...
void cpio_ls(){
    ...
    while(1) {
        ...
        uart_sync_write(info.name, info.namesize);
        uart_puts("\r\n");
...
void cpio_cat(){
    ...
    if (!_strncmp(info.name, input, info.namesize-1)){
        uart_sync_write(info.file, info.filesize);
        uart_puts("\r\n");
    }

}

void parse_cpio_header(cpio_newc_header* header, cpio_ext *info) {
    
    /*uart_puts("[parse_cpio_header] Magic ");
    uart_sync_write(header->c_magic, 6);
    uart_puts("\r\n");*/
