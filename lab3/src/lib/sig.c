...
sig_callback registered_handler[MAX_THREAD][MAX_SIG];
sig_callback default_handler[MAX_THREAD][MAX_SIG];

void sig_init(){
    
    default_handler[0][SIGKILL] = thread_exit;
    
    for(int i=0; i<MAX_THREAD; i++)
        contexts->sys_regs = malloc(sizeof(sys_reg));
}
...
void sig_handler(){
    ...
    {
        int SIG = s->SIG;
        if (registered_handler[curr][SIG]>0){
        
            uint64_t sp_el0 = malloc(THREAD_SP_SIZE) + THREAD_SP_SIZE;
            uint64_t ret_sp, ret_fp;
            asm volatile("mov %0, sp":"=r"(ret_sp));
            asm volatile("mov %0, fp":"=r"(ret_fp));

            call_regis_handler(registered_handler[curr][SIG], sp_el0, ret_sp, ret_fp);

            free(sp_el0-THREAD_SP_SIZE);
            
        } else {
            default_handler[curr][SIG]();
        }
    }
...
void set_sig_handler(int SIG, uint64_t handler){
    int curr = current_thread()->tpid;
    registered_handler[curr][SIG] = handler;
}

void cp_sig_handler(int parent, int child){
    memcpy(&registered_handler[child], &registered_handler[parent], sizeof(sig_callback)*MAX_SIG);
    memcpy(&default_handler[child], &default_handler[parent], sizeof(sig_callback)*MAX_SIG);
}
