...
handler handler_func[SYS_N]={ \
    ...
    signal_regis_handler, kill_handler, sigreturn_handler \
};
...
void uart_write_handler(uint64_t* tf){
    ...
    size_t w_size = uart_puts_withSize(buf, size);
...
void exec_handler(uint64_t* tf){
    ...
    char** argv = tf[1];
    uint64_t lr, sp;
    int res = load_new_image(name, argv, &lr, &sp);
    tf[0] = res;
    //uart_puts(" load fin\r\n");
    from_el1_to_el0(lr, sp);
